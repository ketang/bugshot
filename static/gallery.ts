...
interface GalleryIndexUnit {
    ...
    primary_asset: GalleryAsset;
    vizdiff?: VizdiffMetadata;
}

interface DetailPagePayload {
    unit: GalleryDetailUnit;
    units: Array<{ id: string; label: string; encoded_id: string }>;
    nav: {
        prev: string | null;
        next: string | null;
...
interface VizdiffMetadata {
    classification: "changed" | "added" | "removed" | "unchanged";
    relative_path: string;
    surface?: string;
...
type ToolMode = "off" | "rect" | "ellipse" | "path";
...
type Region = RectRegion | EllipseRegion | PathRegion;
...
type DrawState =
    | { type: "rect"; origin: Point; current: Point }
    | { type: "ellipse"; origin: Point; current: Point }
...
(function () {
    ...
    function toggleVizdiffUnchangedFilter() {
        ...
        if (chip) {
            (chip as HTMLElement).dataset.active = String(!!vizdiffFilters.unchanged);
        }
        renderIndexTiles();
    }
    ...
    function initVizdiffDetailModes(unit) {
        ...
        var overlayToggle = document.getElementById("overlay-toggle") as HTMLInputElement | null;
        if (overlayToggle) {
    ...
    function overlayEnabled() {
        ...
        return !!(toggle && (toggle as HTMLInputElement).checked);
    }
    ...
    function setupRegionDrawing(unit: GalleryDetailUnit, assetsContainer: HTMLElement): RegionState {
        ...
        var card = assetsContainer.querySelector(".asset-card") as HTMLElement | null;
        var image = card ? card.querySelector("img") as HTMLImageElement | null : null;
        if (!card || !image) {
            return state;
        }
        ...
        Array.prototype.slice.call(toolbar.querySelectorAll(".btn-tool")).forEach(function (btn) {
            btn.addEventListener("click", function () {
                setActiveTool(state, btn.getAttribute("data-tool") as ToolMode);
            });
        });
    ...
    function setActiveTool(state: RegionState, tool: ToolMode) {
    ...
    function pointFromImageEvent(state: RegionState, event: MouseEvent): Point | null {
    ...
    function onCardHover(state: RegionState, event: MouseEvent) {
    ...
    function onCardLeave(state: RegionState) {
        if (state.assetCard) {
            state.assetCard.classList.remove("region-hover-active");
        }
        setHighlightedSelection(state, null);
    ...
    function onMouseDown(state: RegionState, event: MouseEvent) {
        if (state.activeTool === TOOL_OFF) {
            return;
        }
        event.preventDefault();
    ...
    function onMouseMove(state: RegionState, event: MouseEvent) {
        if (!state.drawState) {
            return;
        }
        var p = pointFromEvent(state, event);
    ...
    function onMouseUp(state: RegionState, _event: MouseEvent) {
        if (!state.drawState) {
            return;
        }
    ...
    function renderDrawState(state: RegionState) {
        if (!state.drawState) {
            redraw(state);
            return;
        }
    ...
    function previewFromDrawState(drawState: DrawState): Region {
    ...
    function commitDrawState(state: RegionState) {
    ...
    function buildCommittedRegion(drawState: DrawState): Region | null {
        if (drawState.type === TOOL_RECT) {
            var preview = previewFromDrawState(drawState) as RectRegion;
        ...
        if (drawState.type === TOOL_ELLIPSE) {
            var ellipsePreview = previewFromDrawState(drawState) as EllipseRegion;
            if (
    ...
    function applyRegionReplace(state: RegionState, newRegion: Region) {
    ...
    function initDetail() {
        ...
        commentForm.addEventListener("submit", function (event) {
            ...
            var payload: { unit_id: string; body: string; region?: unknown } = { unit_id: currentUnit.id, body: body };
            if (submittedRegion) {
