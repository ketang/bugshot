...
def test_ellipse_region_markdown_summary(tmp_path):
    db = tmp_path / "session.db"
    _seed_db(str(db))
    region = {"type": "ellipse", "cx": 0.4, "cy": 0.55, "rx": 0.12, "ry": 0.08}
    _insert(str(db), "alpha.png", "Spinner offset", region)
    ...
    output = out.getvalue()
    assert "Region: ellipse" in output
    assert "cx=0.40" in output
    assert "rx=0.12" in output
